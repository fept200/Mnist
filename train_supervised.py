...
class DnnAutoEncoder(chainer.Chain):
    ...
    def __call__(self, x):
        h = F.dropout(F.relu(self.e1(x)), ratio=0.1)
        h = F.dropout(F.relu(self.e2(h)), ratio=0.1)
        h = F.dropout(F.relu(self.e3(h)), ratio=0.1)
...
if __name__ == '__main__':
    ...
    """
    gpu = 0
    train_datadir = './data/csv.28x28/train/06_case15'
    ...
    for x in range(50):
        ...
        for f in train_datafiles:
            ...
            hed, hl = model(xs)
            if f[0:1] == '0':
                ts = xp.asarray([0], dtype=np.int32)
            loss = F.softmax_cross_entropy(hl, ts)
        ...
        try:
            print('train ' + str(x + 1) + ':' + str(accum_loss/len(train_datafiles)))
        except ZeroDivisionError:
            print('ZeroDivisionError occuri!!!')

        ## Save Model
        serializers.save_npz('./' + 'model_supervised.npz', copy.deepcopy(model).to_cpu())

        ## 学習終了判定
        #if float(accum_loss/len(train_datafiles)) < 0.00001:
        #    break
