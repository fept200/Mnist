...
"""
DNN AutoEncoder
時系列(OneHot)のデータの特徴値を次元削減で教師ありなし学習する
"""
import numpy as np
import chainer
...
if __name__ == '__main__':
    ...
    """
    gpu = 0
    train_datadir = './data/csv.28x28/train/06'
    ...
    for x in range(100):
        ...
        for f in train_datafiles:
            ...
            hed, hd, hl = model(xs)
            if f[0:1] == '0':
                _loss = model.loss_mean_squared(hd, xs)
                ts = xp.asarray([0], dtype=np.int32)
            loss = F.softmax_cross_entropy(hl, ts) + _loss
        ...
        ## Save Model
        serializers.save_npz('./' + 'model_supervised_unsupervised.npz', copy.deepcopy(model).to_cpu())
